...
function onLoggin (store, token) {
    ...
    if (store.getters.firstTime) {
        router.replace({ name: 'edit-profile', params: {id: 'me'} });
    } else {
        // router.replace({ name: 'search' });
...
function register (store, { community, gender, active, mobile, email, password, passwordConfirmation, name, birthday, termsAndConditions }) {
    let data = {};
    data.gender = gender;
    data.email = email;
